...
def show_max_min(data, key = None, filters = []):
    # Apply filters to data
    for condition in filters:
        data = filter_data(data, condition)

    if key:
        print "data:{} max:{}, min:{}".format(key, data[key].max(), data[key].min())
    else:
...
def usage_plot(data, key = '', filters = [], **kwargs):
    ...
    plt.xlabel(key_name+"'s Time")
    plt.ylabel("Number of People")
    plt.title("Number of People by {:s}'s Time".format(key_name))
    plt.show()